...
- Current runtime/code head: `cd302975013a3d6edc42145d331ff21d2011301f`
...
- `Outfitter.toc` owns the development version: `## Title: Outfitter-dev`, `## Version: 2.0.16-dev`. `OutfitterStrings.lua` reads that version through `GetAddOnMetadata("Outfitter", "Version")`; the old hardcoded upstream `1.4` display value is removed.
...
### Last Runtime Test
- Version/commit: `2.0.15-dev`, exact runtime/code head `a6d7ca70cbec8a29fa5ec9e850d1785e7e6f4961`.
- Result: the hitch is still present, but user felt it was better than before.
- Manual ring change remained detected normally; selecting the saved preset afterward correctly restored the preset's saved rings.
- Interpretation: removing per-slot full reconciliation reduced cost without breaking manual/external equipment observation, but another synchronous refresh cost remains.
- Separate inherited unchecked-slot save/restore bug remains confirmed and unfixed.

### Next Runtime Test
- Test `2.0.16-dev`, exact runtime/code head `cd302975013a3d6edc42145d331ff21d2011301f`.
- Swap one of the larger normal outfits several times and use Birthday Suit several times with the Outfitter window open, because this slice specifically coalesces bag-driven visible-list refreshes.
- Main question: compared with `2.0.15-dev`, is the brief game hitch gone or materially smaller?
- Make one manual ring change afterward and then select the saved outfit again; Outfitter should still notice the manual change and the saved outfit should still restore its own saved rings.
- The unchecked-slot save/restore bug is deliberately unchanged in this build and remains the next isolated correctness slice once the hitch work is settled.
...
- **P3 — cursor-free executor/performance:** physical-executor slices 1–4 remain user-accepted through `2.0.11-dev`. The `2.0.12/2.0.13` timing experiment remains rejected. `2.0.15-dev` reduced redundant per-slot full reconciliation and felt better but did not eliminate the hitch. `2.0.16-dev` separately coalesces bag-driven visible-list refreshes to `BAG_UPDATE_DELAYED`, keeping swap logic and timing constants unchanged.
...
## Exact Next Step
Runtime-test `2.0.16-dev`, exact runtime/code head `cd302975013a3d6edc42145d331ff21d2011301f`, using a large outfit and Birthday Suit with the Outfitter window open. Compare the brief hitch directly with `2.0.15-dev`; this slice only coalesces bag-driven visible-list refreshes to `BAG_UPDATE_DELAYED`. Recheck one manual ring change plus preset restore to confirm external/manual observation still behaves normally. If this materially reduces the hitch, accept the observation/UI-refresh performance work, then fix the inherited unchecked-slot outfit-update bug as its own numeric-version slice.
