...
- Result: P3 slice 4 accepted.

### Next Runtime Test
- No runtime gate remains open for accepted `2.0.11-dev`.
- Physical executor modernization has reached the safe boundary of the current public ClassicAPI surface.
- The next P3 work is a separate audit of the legacy 1.5-second equipment-update throttle and 0.25-second OnUpdate retry loop. Do not change cursor fallback coverage in the same build.
- Keep explicit unequip and weapon-specific multi-step rotations on the accepted legacy executor unless ClassicAPI later exposes a safe direct primitive or a focused design proves a transaction-safe sequence.
...
- **P3 — cursor-free executor:** physical-executor slices 1–4 are user-accepted through `2.0.11-dev`. Further direct migration is intentionally stopped at the current public-API safety boundary: explicit unequip lacks a cursor-free public destination-bag primitive, and weapon-specific multi-step rotations can be affected by weapon-swap timing/GCD rules. Next P3 slice is the separately testable legacy timing audit.
...
## Exact Next Step
Audit the legacy 1.5-second equipment-update throttle and 0.25-second OnUpdate retry loop against the accepted `2.0.11-dev` executor. Identify exactly which paths still depend on each timer, distinguish direct ClassicAPI operations from legacy cursor fallback, and choose the smallest timing change that removes unnecessary delay without weakening fallback correctness. Do not broaden cursor-free executor coverage in that build. Any runtime timing change must receive a numeric version bump and the canonical Lua 5.0.2 compiler check before user testing.
