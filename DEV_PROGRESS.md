...
- P3 slices 1, 2, and 3 physical execution are user-accepted. `2.0.10-dev` proves an exact two-item reciprocal paperdoll exchange can complete with one atomic direct swap after verifying both source GUIDs. Explicit unequip, bank-sourced, non-reciprocal slot-to-slot dependency/source, moved/ambiguous source, mixed, and no-GUID cases remain cursor-driven.
...
### Next Runtime Test
- No runtime gate remains open for accepted `2.0.10-dev`.
- The next runtime build must increment the numeric version.
- Keep `2.0.10-dev` as the known-good reciprocal-slot baseline while auditing the smallest non-reciprocal paired-slot rotation that can be expressed entirely with atomic swaps.
- Explicit unequips remain legacy unless a public ClassicAPI operation can move an equipped item to a known bag destination without hidden/shared equipment-set storage.
...
- **P3 — cursor-free executor:** active. Slices 1–3 are user-accepted through `2.0.10-dev`. Remaining cursor-driven classes are explicit unequip, non-reciprocal paired-slot rotations/chains, bank paths, moved/ambiguous sources, mixed unsupported lists, and no-GUID fallback. Next audit should choose the smallest safe non-reciprocal paired-slot case before considering timing cleanup.
...
## Exact Next Step
Audit the optimized paired-slot change patterns after accepted `2.0.10-dev`, especially the case where one equipped ring/trinket/compatible weapon moves to its paired slot and a normal-bag item replaces its old slot. Determine whether that pattern can be prevalidated and completed as paperdoll swap first, then bag->paperdoll swap, with no explicit unequip and no re-resolution after packets are sent. If and only if the pattern is unambiguous and independent, implement it as the next narrow P3 slice with a numeric version bump and Lua 5.0.2 compiler check. Keep explicit unequip and legacy timing unchanged.
