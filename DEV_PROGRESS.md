...
- P3 slice 1 physical execution is user-accepted: a single exact non-bank equip/replacement can use the cursor-free ClassicAPI path. `2.0.8-dev` showed that acknowledgement-by-acknowledgement multi-change sequencing adds visible per-slot latency. `2.0.9-dev` removes that wait for the same narrow safe all-normal-bag replacement case by validating all exact source GUID/locations first and then bursting atomic direct swaps; this is now user-accepted. Explicit unequip, bank-sourced, slot-to-slot dependency/source, moved/ambiguous source, mixed, and no-GUID cases remain cursor-driven.
...
### Next Runtime Test
- No runtime gate is open for the accepted `2.0.9-dev` slice.
- The next P3 implementation must receive a new numeric dev version before testing.
- Keep `2.0.9-dev` as the known-good baseline while deciding the next executor slice.
- Do not conflate the next executor expansion with removal/retuning of the legacy 1.5-second throttle or 0.25-second retry loop unless that timing change is itself the explicitly chosen, separately testable slice.
...
- **P3 — cursor-free executor:** active. Slice 1 and slice 2 are user-accepted. Known-good current baseline is `2.0.9-dev` at runtime/code head `0e2ec6fbea2a4f0fda70dbdf13e3f9c98b4df3b9`: single exact replacements and clean all-normal-bag multi-replacement lists can use cursor-free ClassicAPI swaps, with the latter prevalidated then burst immediately. Remaining cursor-driven cases include explicit unequip, slot-to-slot dependency/source, bank, moved/ambiguous source, mixed, and no-GUID lists. The next P3 slice should target one of those remaining executor classes or, separately, the legacy timing machinery; do not combine both in one test build.
...
## Exact Next Step
Audit the remaining P3 cursor-driven executor cases against the accepted `2.0.9-dev` baseline and choose the smallest next migration slice. Prefer completing physical-executor coverage (slot-to-slot dependencies and/or explicit unequips using ClassicAPI's atomic swap primitives with precomputed source/destination state) before changing the legacy 1.5-second throttle or 0.25-second retry loop, unless inspection shows those timing mechanisms independently block the next executor slice. Do not change both executor coverage and timing behavior in the same runtime build. Any new runtime code must bump to the next numeric dev version and receive the canonical Lua 5.0.2 compiler check before user testing.
