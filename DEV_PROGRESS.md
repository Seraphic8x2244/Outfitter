...
- Current runtime/code head: `c81bf220ca90ebe65f57ba89795c51d2a491e1ba`
...
### Last Runtime Test
- Version/runtime head: `2.0.22-dev`, `590973252cd352a0e827be8b761fbc77b3377442`.
- User screenshots confirm the Outfits page loads and renders with the requested inset-left title, top-right `New Outfit` placement, 95x25 button proportions, and unchanged outfit-list position.
- Accepted visually: horizontal title/button spacing and button size.
- Follow-up requested: move both controls down together to the vertical centre of the header; remove the Outfits-page-only grey bottom bar so the active tab joins the page like Options; rename the tab `Outfits`; show the outer version as `v<version>`.
- No explicit `New Outfit` click-result was reported in this screenshot pass; keep that functional click in the next runtime check.

### Next Runtime Test
- Runtime-check `2.0.24-dev` UI only: confirm the bottom tab reads `Outfits`; when selected, it joins directly into the page like `Options` with no extra light-grey strip/border; the inner `Outfitter` title and 95x25 `New Outfit` button keep their accepted horizontal positions but are vertically centred in the header band; the outfit list has not moved; the outer title shows `v2.0.24-dev`; and `New Outfit` still works when clicked.
...
## Exact Next Step
Runtime-check `2.0.24-dev` UI only: verify the active bottom tab is labelled `Outfits` and joins the page like `Options` with no extra light-grey bar/border; the `Outfitter` title and 95x25 `New Outfit` button retain their accepted X positions but are vertically centred together; the outfit list remains unmoved; the outer frame shows `v2.0.24-dev`; and `New Outfit` still functions. If accepted, resume P4 with Riding as the first single-consumer slice using `OutfitterClassicAPI.GetRidingState()` when available while retaining the existing aura/tooltip Riding heuristic as the Turtle/non-capability fallback and validation protection. Do not yet consume the helpful-aura or shapeshift bridge, do not add Swimming or Turtle-only Tree/Swift Travel behavior, and do not touch Dining health/mana semantics, zone behavior, the residual hitch, 1.5-second throttle, or 0.25-second equipment retry loop.
