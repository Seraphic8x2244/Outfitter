...
- Not yet confirmed for P3 slice 1: the same glove swap in the reverse direction and repeated several times; confirmation that Outfitter's UI displays `0.1.1-dev` from TOC metadata.
...
- Confirm the UI reports `0.1.1-dev` from the TOC.
- Starting from the enchanted Gauntlets of the Righteous Champion, switch to the one-slot outfit containing the unenchanted pair, then switch back and forth several times. Confirm the intended pair equips every time and no Lua errors occur.
...
## Exact Next Step
Finish the P3 slice 1 runtime gate on version `0.1.1-dev`, exact runtime/code head `026bd1164d817dd6e4775f76df40dd293b2492fa`: confirm the UI displays `0.1.1-dev`, then starting from the enchanted Gauntlets of the Righteous Champion switch to the one-slot outfit containing the unenchanted pair and repeat swaps in both directions several times. The manual character-screen swap and ordinary full-set swap already passed on this build. Report the remaining result before any further P3 implementation. Do not begin event-driven multi-change sequencing or alter the 1.5-second throttle / 0.25-second retry loop until this slice is user-verified.
