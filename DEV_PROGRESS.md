...
- Current runtime/code head: `cec8bf4e3421f2b42f5486860af0141ef8719493`
...
- `Outfitter.toc` owns the development version: `## Title: Outfitter-dev`, `## Version: 2.0.17-dev`. `OutfitterStrings.lua` reads that version through `GetAddOnMetadata("Outfitter", "Version")`; the old hardcoded upstream `1.4` display value is removed.
...
### Last Runtime Test
- Version/commit: `2.0.16-dev`, exact runtime/code head `cd302975013a3d6edc42145d331ff21d2011301f`.
- Result: the brief hitch is still present and felt about the same as `2.0.15-dev`.
- Manual/ring behavior remained correct.
- Interpretation: coalescing only bag-driven visible-list refreshes did not materially change the hitch; the remaining cost is deeper in legacy inventory reconciliation.
- Separate inherited unchecked-slot save/restore bug remains confirmed and unfixed.

### Next Runtime Test
- Test `2.0.17-dev`, exact runtime/code head `cec8bf4e3421f2b42f5486860af0141ef8719493`.
- Use the same large outfit and Birthday Suit tests as before. Main question: is the brief client hitch gone or materially smaller now that repeated legacy inventory-change events reconcile only once per frame?
- Recheck one manual ring change and then select the saved outfit again; Outfitter should still notice the manual change and restore the saved rings normally.
- No need to test unchecked-slot behavior yet; that correctness bug remains deliberately unchanged.
...
- **P3 — cursor-free executor/performance:** physical-executor slices 1–4 remain user-accepted through `2.0.11-dev`. The `2.0.12/2.0.13` timing experiment remains rejected. `2.0.15-dev` reduced redundant ClassicAPI per-slot reconciliation and felt better but did not eliminate the hitch. `2.0.16-dev` bag-UI coalescing was neutral. `2.0.17-dev` now coalesces repeated legacy `UNIT_INVENTORY_CHANGED` reconciliation to one pass per frame, still without changing swap logic or timing constants.
...
## Exact Next Step
Runtime-test `2.0.17-dev`, exact runtime/code head `cec8bf4e3421f2b42f5486860af0141ef8719493`, using the same large outfit and Birthday Suit cases. Compare the hitch directly with `2.0.16-dev`; this slice only defers and coalesces repeated legacy `UNIT_INVENTORY_CHANGED` reconciliation to one next-frame pass. Recheck one manual ring change plus preset restore to confirm external/manual observation remains correct. If this still does not materially reduce the hitch, stop making further speculative performance changes and add lightweight instrumentation to measure which remaining path consumes the frame before changing behavior again.
