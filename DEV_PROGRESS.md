...
- `2.0.18-dev` is implemented and compiler-checked at exact runtime/code head `de0ddd8468dab5d27388ab178cc4af432ed5ec78`. It changes only the selected-outfit reconciliation condition so unchecked or unknown slots are not silently saved back into the outfit. Awaiting in-game validation.
...
- The inherited unchecked-slot outfit-editor bug is fixed in untested `2.0.18-dev`: `Outfitter_UpdateOutfitFromInventory` now updates only slots whose checkbox is checked and not unknown. Runtime validation is still required before accepting the fix.
...
### Last Runtime Test
- Version/commit: `2.0.17-dev`, exact runtime/code head `cec8bf4e3421f2b42f5486860af0141ef8719493`.
- Result: the outfit-change hitch is still present but is noticeably shorter than `2.0.16-dev`.
- Interpretation: per-frame coalescing of legacy `UNIT_INVENTORY_CHANGED` reconciliation materially reduces the hitch and is accepted for its performance goal.
- Manual/external ring observation was not separately re-reported on `2.0.17-dev`; include that sanity check in the next gate.

### Next Runtime Test
- Test `2.0.18-dev`, exact runtime/code head `de0ddd8468dab5d27388ab178cc4af432ed5ec78`.
- Select an outfit and uncheck one slot, preferably one ring slot. Manually change the item in that unchecked slot while the outfit is selected. After reconciliation and reopening/reselecting the outfit, that slot must still be unchecked; Outfitter must not silently save the new item into that outfit.
- Apply that outfit again after manually changing the unchecked slot. The outfit must leave that slot alone unless another active outfit in the stack legitimately owns it.
- As a control, manually change an item in a checked, known slot while the outfit is selected. Existing Outfitter behavior should still allow that checked slot to update in the selected outfit.
- Recheck one ordinary outfit swap and one manual ring change/preset restore. The noticeably shorter `2.0.17-dev` hitch behavior should remain unchanged because `2.0.18-dev` does not alter the performance path.
...
- **P3 — cursor-free executor/performance:** physical-executor slices 1–4 remain user-accepted through `2.0.11-dev`. The `2.0.12/2.0.13` timing experiment remains rejected. `2.0.15-dev` reduced redundant ClassicAPI per-slot reconciliation, `2.0.16-dev` bag-UI coalescing was neutral, and `2.0.17-dev` materially shortened the hitch by coalescing legacy inventory reconciliation once per frame. Keep the remaining hitch as profiling debt; do not resume speculative timing changes while testing the isolated `2.0.18-dev` unchecked-slot correctness fix.
...
## Exact Next Step
Runtime-test `2.0.18-dev`, exact runtime/code head `de0ddd8468dab5d27388ab178cc4af432ed5ec78`. Verify an unchecked outfit slot remains unchecked and is not silently re-saved when that physical slot changes; verify a checked, known slot still updates normally when edited; then do one ordinary outfit swap plus manual ring change/preset restore to confirm `2.0.17-dev` observation/performance behavior is retained. Do not make further hitch/timing changes during this correctness gate.
