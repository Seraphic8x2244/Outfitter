...
- Current runtime/code head: `96a36579c070a7e626b57d1d5f389b5a3110c315`
...
### Last Runtime Test
- Version: `2.0.20-dev`; exact runtime/code head `1cbac5b2dc505a7d04266628ce597c76c0877e86`.
- Failed structural smoke: clicking the Outfitter character-frame button raised a nil-global error for `Outfitter_ToggleOutfitterFrame`.
- Static trace: the function is defined globally later in `Outfitter.lua`, so the click error is secondary and means the chunk stopped before that definition. The structural cleanup had changed locale TOC entries from root files to forward-slash `locales/...` paths; locale-dependent top-level tables appear before the toggle function, while XML can still load afterward. `2.0.21-dev` therefore changes only those TOC entries to Vanilla-style backslashes and requires retest before any P4 consumer integration.
- Previous accepted runtime baseline remains `2.0.18-dev`, exact runtime/code head `de0ddd8468dab5d27388ab178cc4af432ed5ec78`.

### Next Runtime Test
- Smoke-test `2.0.21-dev`: confirm no Lua errors during load or when clicking the Outfitter button, confirm the UI reports `2.0.21-dev`, open Outfitter normally, perform a normal saved-outfit swap, and make one manual gear change. This specifically verifies the locale loader-path correction and the original single-file/runtime-locales structural cleanup. The P4 fact bridge remains intentionally unused, so this is still a structural/regression gate.
...
## Exact Next Step
Runtime-smoke-test `2.0.21-dev`: confirm no Lua errors during load or on the Outfitter button, confirm the UI reports `2.0.21-dev`, open Outfitter normally, perform a normal saved-outfit swap, and make one manual gear change. Do not integrate any P4 consumer until this structural gate passes. If it passes, integrate Riding as the first single-consumer P4 slice using `OutfitterClassicAPI.GetRidingState()` when available while retaining the existing aura/tooltip Riding heuristic as the Turtle/non-capability fallback and validation protection. Do not yet consume the helpful-aura or shapeshift bridge, do not add Swimming or Turtle-only Tree/Swift Travel behavior, and do not touch Dining health/mana semantics, zone behavior, the residual hitch, 1.5-second throttle, or 0.25-second equipment retry loop.
