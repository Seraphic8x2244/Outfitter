...
- Current P4 candidate: `2.0.27-dev`, runtime/code head `ba029badd87b2732fc5e2d8436cffdb6ca621eff`, is the first Riding consumer. `Outfitter_GetPlayerAuraStates()` now reads `OutfitterClassicAPI.GetRidingState()` first; a boolean result is authoritative for the existing `Riding` state, while the imported Cosmin/Turtle mount-tooltip heuristic remains available only when that helper returns `nil`. `PLAYER_AURAS_CHANGED`, special-outfit IDs, stack/priority behavior, initialization/reset behavior, Dining, shapeshift handling, zones, equipment timing, and the helper implementations themselves are otherwise unchanged. This slice is compiler-checked but not yet runtime-tested.
