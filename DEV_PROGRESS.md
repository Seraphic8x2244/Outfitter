...
- Current runtime/code head: `f998bdd9b474bd46fbc42f749896a9a69a8bad59`
...
### Next Runtime Test
- Runtime-check `2.0.25-dev` UI only: confirm one additional outfit row is visible at the bottom (15 visible rows total when enough entries are present), the list still begins at the same top position, the scrollbar/trench extends down to match the extra row without overshooting or overlapping the tabs, scrolling still reaches the first/last entries correctly, and the accepted `2.0.24-dev` header/tab/version layout remains unchanged.
...
## Exact Next Step
Runtime-check `2.0.25-dev` UI only: verify one additional outfit row is visible at the bottom (15 total when enough entries are present), the list top remains unchanged, the scrollbar/trench extends downward by the same one-row height without colliding with the tabs, and scrolling still reaches both ends correctly. Confirm the accepted `2.0.24-dev` header/tab/version chrome is unchanged. If accepted, resume P4 with Riding as the first single-consumer slice using `OutfitterClassicAPI.GetRidingState()` when available while retaining the existing aura/tooltip Riding heuristic as the Turtle/non-capability fallback and validation protection. Do not yet consume the helpful-aura or shapeshift bridge, do not add Swimming or Turtle-only Tree/Swift Travel behavior, and do not touch Dining health/mana semantics, zone behavior, the residual hitch, 1.5-second throttle, or 0.25-second equipment retry loop.
