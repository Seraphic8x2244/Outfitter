...
- Current runtime/code head: `d10f7d26b37818671e0bd8c8ef205b10f4eef595`
...
- `Outfitter.toc` owns the development version: `## Title: Outfitter-dev`, `## Version: 2.0.13-dev`. `OutfitterStrings.lua` reads that version through `GetAddOnMetadata("Outfitter", "Version")`; the old hardcoded upstream `1.4` display value is removed.
...
- `2.0.13-dev` timing diff keeps all swap-selection rules from accepted `2.0.11-dev`. `Outfitter_ExecuteEquipmentChangeList` now reports direct versus legacy execution; all direct adapter paths register expected final slot/GUID pairs before sending; `ObservePlayerEquipmentChanged` clears each expected slot only when the live GUID matches; unresolved direct work retains the old 1.5-second value only as a timeout ceiling; legacy cursor execution still sets the fixed 1.5-second delay. The 0.25-second OnUpdate retry scheduler is unchanged.
- Pre-handoff review caught that the first `2.0.12-dev` revision omitted pending-slot registration for reciprocal direct swaps. `2.0.13-dev` adds both reciprocal final slots before the direct packet and supersedes `2.0.12-dev` without runtime testing.
- Real Lua 5.0.2 compiler validation passed all 8 runtime Lua files in GitHub Actions run `36167384692` on validation commit `80f927b8f899f20ad8d6d915c47844df862f848b`; its addon runtime files exactly match runtime/code head `d10f7d26b37818671e0bd8c8ef205b10f4eef595`. The temporary workflow was removed at `f73993a6d9fa4a85625d2a5899775bdbaf3fa566`.
...
- P3 slices 1–4 physical execution are user-accepted through `2.0.11-dev`. Safe public-API direct coverage now includes single exact replacements, independent normal-bag multi-replacements, exact reciprocal paired-slot exchanges, and the exact one-way ring/trinket rotation with bag replacement. Explicit unequip remains cursor-driven because ClassicAPI exposes no public equipped-slot -> chosen-bag-slot direct operation. Weapon-specific multi-step rotations remain legacy because weapon-swap timing/GCD restrictions can make a two-step direct sequence partially succeed. Bank, moved/ambiguous source, mixed unsupported, and no-GUID cases also remain legacy. `2.0.13-dev` changes timing/coordination only: direct operations now wait on their expected final equipment events rather than inheriting the blanket 1.5-second legacy lock.
...
### Last Runtime Test
- Version/commit: `2.0.11-dev`, exact runtime/code head `9353beaa09498e318a1a174984cbdd270a18cea2`.
- Passed: the targeted one-way ring/trinket paired-slot rotation with a normal-bag replacement works.
- No issue was reported.
- Result: P3 physical-executor slice 4 accepted.

### Next Runtime Test
- Test `2.0.13-dev`, exact runtime/code head `d10f7d26b37818671e0bd8c8ef205b10f4eef595`.
- Focus on rapid successive **direct-path** outfit requests: switch between ordinary bag-sourced outfits faster than 1.5 seconds. The second requested state should begin once the prior direct paperdoll changes reconcile, rather than waiting the old fixed 1.5 seconds. The unchanged 0.25-second scheduler may still make a queued follow-up visibly wait up to roughly one polling interval.
- Repeat rapid reciprocal ring swaps and the accepted ring/trinket rotation. Final physical items must remain correct with no bag loss, wrong slot, Lua error, or server equip error.
- Then exercise a known **legacy fallback** such as an explicit empty-slot unequip or unsupported weapon case and request another outfit immediately afterward. That legacy path should remain protected by the old 1.5-second delay and still resolve correctly.
- Make one manual or pfUI equipment change afterward and confirm it remains external/manual.
- If a direct transaction event never fully reconciles, the 1.5-second ceiling must prevent a permanent lock and allow later work to continue.
- Do not remove or retune the 0.25-second scheduler in this gate.
...
- **P3 — cursor-free executor/timing:** physical-executor slices 1–4 are user-accepted through `2.0.11-dev`. Further direct migration is intentionally stopped at the current public-API safety boundary. Timing slice 1 is implemented and compiler-checked in corrected `2.0.13-dev`: accepted direct operations are event-unlocked by expected final slot/GUID reconciliation, while the old 1.5-second fixed delay remains for legacy cursor execution and as a failure ceiling for unresolved direct transactions. The 0.25-second retry scheduler is deliberately unchanged for this gate.
...
## Exact Next Step
Runtime-test `2.0.13-dev`, exact runtime/code head `d10f7d26b37818671e0bd8c8ef205b10f4eef595`. Rapidly request successive accepted direct-path outfits inside the old 1.5-second window and verify the queued state proceeds after the prior expected paperdoll GUIDs reconcile, with correct final items and no errors. Recheck reciprocal ring and accessory-rotation direct paths, then exercise one legacy cursor fallback and confirm its fixed 1.5-second protection remains. Confirm manual/pfUI changes remain external. Do not change the 0.25-second retry scheduler until this timing slice is user-verified.
