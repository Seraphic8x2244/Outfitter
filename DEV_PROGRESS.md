...
- Current runtime/code head: `590973252cd352a0e827be8b761fbc77b3377442`
...
- `2.0.22-dev` changes only `Outfitter.xml` header layout plus the TOC version: inner title anchor `LEFT`/`TOPLEFT` at `(42,-36)`; `New Outfit` is 95x25 and anchors `RIGHT`/`TOPRIGHT` at `(-14,-36)`. The outfit list/scroll anchors are unchanged.
...
### Last Runtime Test
- Version/runtime head: `2.0.21-dev`, `96a36579c070a7e626b57d1d5f389b5a3110c315`.
- Passed: no Lua error during addon load or when clicking the Outfitter character-frame button.
- Passed: Outfitter opens normally and displays `2.0.21-dev`.
- Passed: normal saved-outfit switching and manual gear-change reconciliation behave normally.
- Result: the single-file/runtime-locales structural cleanup and Vanilla-style locale loader-path correction are accepted. The previous `2.0.20-dev` forward-slash TOC path failure is closed.

### Next Runtime Test
- Runtime-check `2.0.22-dev` layout only: confirm the inner `Outfitter` title is inset on the left, `New Outfit` is mirrored on the right on the same visual centre line, the button's 95x25 proportions look right, there is no overlap/squeeze, and the outfit list has not moved. Click `New Outfit` once to confirm the moved control remains functional.
...
## Exact Next Step
Runtime-check `2.0.22-dev` header layout: verify the inset-left `Outfitter` heading and top-right 95x25 `New Outfit` button share the same visual centre line, do not overlap, and leave the outfit list in its existing position; click `New Outfit` once. If accepted, resume P4 with Riding as the first single-consumer slice using `OutfitterClassicAPI.GetRidingState()` when available while retaining the existing aura/tooltip Riding heuristic as the Turtle/non-capability fallback and validation protection. Do not yet consume the helpful-aura or shapeshift bridge, do not add Swimming or Turtle-only Tree/Swift Travel behavior, and do not touch Dining health/mana semantics, zone behavior, the residual hitch, 1.5-second throttle, or 0.25-second equipment retry loop.
