...
- Current runtime/code head: `1cbac5b2dc505a7d04266628ce597c76c0877e86`
...
### Last Runtime Test
- Version/commit: `2.0.18-dev`, exact runtime/code head `de0ddd8468dab5d27388ab178cc4af432ed5ec78`.
- Passed: a partial/RP outfit with only visible armour selected leaves unticked ring slots unchanged.
- Passed: full outfit A -> RP -> full outfit B -> RP leaves the rings from the most recent full outfit in place; the RP outfit does not reclaim them.
- Passed: manually interacting with a ring while that RP ring slot is unticked does not silently add the ring to the RP outfit.
- Design conclusion: the checkbox is the explicit slot-ownership control. Unticked means the outfit does not own, remember, or restore that slot; ticking a slot deliberately adds the currently equipped item.
- Result: `2.0.18-dev` unchecked-slot correctness fix accepted.

### Next Runtime Test
- Smoke-test `2.0.20-dev`: confirm no Lua errors, confirm the UI reports `2.0.20-dev`, open Outfitter normally, perform a normal saved-outfit swap, and make one manual gear change to confirm the `2.0.19-dev` single-file/runtime-locales cleanup still behaves as in the accepted `2.0.18-dev` baseline. The new P4 bridge is intentionally unused in this build, so this test remains a structural/regression gate rather than an automatic-state behavior test.
...
## Exact Next Step
Runtime-smoke-test `2.0.20-dev`: confirm the addon loads without Lua errors, reports `2.0.20-dev`, opens normally, and a normal saved-outfit swap plus a manual gear change still behave as in the accepted `2.0.18-dev` baseline. This closes the still-unperformed `2.0.19-dev` structural gate; the `2.0.20-dev` P4 bridge is behavior-neutral and unused. If that gate passes, integrate Riding as the first single-consumer P4 slice using `OutfitterClassicAPI.GetRidingState()` when available while retaining the existing aura/tooltip Riding heuristic as the Turtle/non-capability fallback and validation protection. Do not yet consume the helpful-aura or shapeshift bridge, do not add Swimming or Turtle-only Tree/Swift Travel behavior, and do not touch Dining health/mana semantics, zone behavior, the residual hitch, 1.5-second throttle, or 0.25-second equipment retry loop.
