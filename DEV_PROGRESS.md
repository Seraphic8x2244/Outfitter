...
- Current runtime/code head: `a6d7ca70cbec8a29fa5ec9e850d1785e7e6f4961`
...
- `Outfitter.toc` owns the development version: `## Title: Outfitter-dev`, `## Version: 2.0.15-dev`. `OutfitterStrings.lua` reads that version through `GetAddOnMetadata("Outfitter", "Version")`; the old hardcoded upstream `1.4` display value is removed.
...
### Last Runtime Test
- Version/commit: `2.0.14-dev`, exact runtime/code head `678c3edcc5536744776791cb8a969b75c6b74cc4`.
- Result: the brief game hitch still occurs when changing outfits; user thought it may be somewhat less severe than `2.0.13-dev`.
- This disproves the earlier conclusion that the hitch was caused solely by the rejected event-unlock timing experiment.
- Audit finding: the hitch candidate predates that experiment and exists back to P1, where every ClassicAPI `PLAYER_EQUIPMENT_CHANGED` slot event was routed through the full legacy inventory reconciliation while `UNIT_INVENTORY_CHANGED` was also retained.
- Separate inherited unchecked-slot save/restore bug remains confirmed and unfixed in this gate.

### Next Runtime Test
- Test `2.0.15-dev`, exact runtime/code head `a6d7ca70cbec8a29fa5ec9e850d1785e7e6f4961`.
- Use normal human tests only: swap one of the larger ordinary outfits several times, then equip/unequip Birthday Suit.
- Main question: is the brief game freeze/hitch gone or clearly reduced now that Outfitter no longer performs a full bag/equipment rescan once per changed slot?
- Also make one manual or pfUI equipment change and confirm Outfitter still notices it normally; that should continue through Vanilla's existing inventory event.
- Do not evaluate the unchecked-slot bug in this build; it is deliberately unchanged and remains the next isolated fix after this hitch gate.
...
- **P3 — cursor-free executor/timing:** physical-executor slices 1–4 remain user-accepted through `2.0.11-dev`. The `2.0.12/2.0.13` timing experiment remains rejected. `2.0.14-dev` proved the remaining hitch predates that experiment. `2.0.15-dev` targets the older P1 duplicate-reconciliation cost only, keeping swap logic and timing constants unchanged.
...
## Exact Next Step
Runtime-test `2.0.15-dev`, exact runtime/code head `a6d7ca70cbec8a29fa5ec9e850d1785e7e6f4961`, using a large normal outfit and Birthday Suit. Judge only whether the brief client hitch is gone or materially reduced, and confirm one manual/pfUI equipment change is still detected. This build removes only the duplicate full reconciliation from ClassicAPI's per-slot event; Vanilla `UNIT_INVENTORY_CHANGED` remains the full state owner. If this resolves the hitch, accept the observation-path change and then fix the inherited unchecked-slot outfit-update bug as a separate versioned slice.
